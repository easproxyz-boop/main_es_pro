...
router.post('/table/employee/uSc233/profile_info', isAuthenticated, async (req, res) => {
  try {
    ...
    // Map DataTables columns index to DB columns
    const columnMap = [
      null, // No column for index 0 (row number)
      'dt_date_added',
    ...
    const filteredQuery = `SELECT COUNT(*) as total FROM tbl_employee_profile_info ${where}`;
    ...
    const data = rows.map(row => ({
      ...
      employee_infos: `
        <div class="d-flex align-items-center">
          <img 
            src="${row.dt_employee_picture !== 'NONE' ? row.dt_employee_picture.replace(/\.jxl$/i, '.jpeg') : './assets/images/default-picture/user-3-fill.png'}" 
